...
def _real_doc_summary(text: str, kind: str) -> dict:
    ...
    raw = llm.chat_text(
        system=(
            ...
            "  - Schema: {"
            "\"summary\": <bullet list — 4-8 short bullets, each on its own line "
            "prefixed with '- '. NEVER a paragraph. Each bullet ≤25 words. Present "
            "tense. Cover the substance: who, what, decisions, asks, risks.>, "
...
def _real_vpd_extract(text: str) -> dict:
    ...
    raw = llm.chat_text(
        system=(
            ...
            "{\n"
            "  \"proposed_solution\": <bullet list — 3-6 short bullets, each on "
            "its own line prefixed with '- '. NEVER a paragraph. or null>,\n"
            ...
            "  \"value_definition\": <bullet list — 3-6 short bullets, each on "
            "its own line prefixed with '- ', explaining HOW value is measured. "
            "NEVER a paragraph. or null>,\n"
            "  \"estimated_value_musd\": <number in millions USD or null>\n"
            "}\n"
            "Rules:\n"
            "  - 03-Jun bug — `proposed_solution` and `value_definition` MUST be "
            "bullet format. Each bullet on its own line prefixed with '- '. "
            "Even single-point answers go as one bullet, never as a paragraph.\n"
